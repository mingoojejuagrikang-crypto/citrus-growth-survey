...
function makeInitialSessionFields(): SessionFields {
  return {
...
class SurveyStore extends Observable<SurveyState> {
  constructor() {
   ...
   */
  updateSessionFields(fields: Partial<SessionFields>): void {
    const state = this.getState();
    this.setState({
   ...
   */
  setLastField(field: string | null): void {
   ...
   */
  loadSession(session: SessionFields): void {
  ...
  resetAfterSave(): void {
    ...
    delete (updatedRecord as Record<string, unknown>)['fruitNo'];
    this.setState({
      currentRecord: updatedRecord,
      isDirty: false,
    });
   ...
   */
  private recalculate(record: Partial<QualityRecord>): void {
    // 과피두께×4 자동계산
  ...
  buildRecord(
    ...
    const base: GrowthRecord = {
      ...
      fruitNo: (state.currentRecord as Partial<GrowthRecord>).fruitNo ?? 0,
      width: (state.currentRecord as Partial<GrowthRecord>).width ?? null,
      height: (state.currentRecord as Partial<GrowthRecord>).height ?? null,
      remark: (state.currentRecord as Partial<GrowthRecord>).remark ?? '',
      syncStatus: 'pending',
      syncedAt: null,
      createdAt: now,
      updatedAt: now,
    ...
    if (state.surveyType === 'quality') {
      const qRecord = state.currentRecord as Partial<QualityRecord>;
      return {
        ...base,
