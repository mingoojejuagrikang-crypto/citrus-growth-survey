...
let currentPage: PageInstance | null = null;
...
async function route(hash: string): Promise<void> {
  ...
  if (hash.startsWith('#/settings')) {
    ...
    const p = new SurveyInputPage('quality' as SurveyType);
    await p.mount(pageContainer);
    page = p;

  } else if (hash.startsWith('#/survey/growth') || hash === '#/survey' || hash === '') {
    const p = new SurveyInputPage('growth' as SurveyType);
    await p.mount(pageContainer);
    page = p;
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    // BASE_URL을 사용해 GitHub Pages 하위 경로에서도 올바른 경로로 등록됨
