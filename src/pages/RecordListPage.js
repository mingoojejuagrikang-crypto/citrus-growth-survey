...
export class RecordListPage {
    ...
    async loadRecords() {
        ...
        try {
            ...
            // 타입 태그 추가 (구분용)
            const taggedGrowth = growthRecords.map((r) => ({ ...r, _type: 'growth' }));
            const taggedQuality = qualityRecords.map((r) => ({ ...r, _type: 'quality' }));
            this.allRecords = [
                ...taggedGrowth,
                ...taggedQuality,
    ...
    renderRecordDetail(record, surveyType) {
        const growthR = record;
        const qualityR = record;
        const commonFields = [
    ...
    // ─────────────────────────────────────────────
    mountSyncStatusBar() {
        const container = this.el?.querySelector('#sync-status-container');
        if (!container)
            return;
        this.syncStatusBar?.unmount();
    ...
    bindEvents() {
        ...
        groupsContainer?.addEventListener('click', async (e) => {
            ...
            const surveyType = (actionEl.dataset['surveyType'] ?? 'growth');
    ...
    // 핸들러
    // ─────────────────────────────────────────────
    handleToggleDetail(recordId) {
        this.expandedRecordId = this.expandedRecordId === recordId ? null : recordId;
        this.rerenderGroups();
    }
    async handleDeleteRecord(recordId, surveyType) {
        const confirmed = await showConfirm('레코드를 삭제하면 복구할 수 없습니다. 삭제하시겠습니까?', '삭제', true);
        if (!confirmed)
            return;
        try {
    ...
    handleEditRecord(recordId, surveyType) {
    ...
    handleFilterChange(filter) {
        ...
        this.rerenderGroups();
        // 필터 버튼 활성화 상태 업데이트
        const filterBtns = this.el?.querySelectorAll('[data-filter]');
    ...
    // ─────────────────────────────────────────────
    rerenderGroups() {
        const container = this.el?.querySelector('#record-groups');
        if (!container)
            return;
        const filteredRecords = this.getFilteredRecords();
        const groups = this.groupRecords(filteredRecords);
    ...
    groupRecords(records) {
        ...
        for (const record of records) {
            const tagged = record;
            const surveyType = tagged._type ?? 'growth';
        ...
        for (const group of groupMap.values()) {
            group.records.sort((a, b) => {
                const ga = a;
                const gb = b;
                if (ga.treeNo !== gb.treeNo)
                    return ga.treeNo - gb.treeNo;
                return ga.fruitNo - gb.fruitNo;
            });
        }
        // 날짜 내림차순으로 정렬
        return Array.from(groupMap.values()).sort((a, b) => b.surveyDate.localeCompare(a.surveyDate));
    }
    // ─────────────────────────────────────────────
    // 유틸리티
    // ─────────────────────────────────────────────
