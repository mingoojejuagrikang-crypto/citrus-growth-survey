...
export class SettingsPage {
    ...
    async loadSettings() {
        try {
            [
                ...
                SettingsService.get('ttsEnabled', true),
                SettingsService.get('gemmaEnabled', false),
                SettingsService.get('audioRecordEnabled', false),
                SettingsService.get('voiceLogEnabled', true),
            ]);
        }
        catch {
    ...
    setupGemmaToggle() {
        ...
        const descEl = this.el?.querySelector('#gemma-desc');
        const toggleInput = this.el?.querySelector('#gemma-toggle');
        const toggleRow = this.el?.querySelector('#gemma-toggle-row');
        if (isIOS) {
            if (descEl)
                descEl.textContent = '이 기기에서 AI 보조 기능이 지원되지 않습니다';
            if (toggleInput) {
            ...
            if (descEl)
                descEl.textContent = 'Android Chrome에서 비고 입력 보조 제안 (Gemma AI)';
        }
    }
    // ─────────────────────────────────────────────
    // 이벤트 바인딩
    ...
    bindEvents() {
        ...
        this.el.addEventListener('click', (e) => {
            ...
            switch (action) {
                case 'delete-farmer':
                    if (name)
                        this.handleDeleteFarmer(name);
                    break;
                case 'set-default-farmer':
                    if (name)
                        this.handleSetDefaultFarmer(name);
                    break;
                case 'set-default-label':
                    if (value)
                        this.handleSetDefaultLabel(value);
                    break;
                case 'delete-label':
                    if (value)
                        this.handleDeleteLabel(value);
                    break;
                case 'set-default-treatment':
                    if (value)
                        this.handleSetDefaultTreatment(value);
                    break;
                case 'delete-treatment':
                    if (value)
                        this.handleDeleteTreatment(value);
                    break;
            }
        });
        // 토글 변경
        const ttsToggle = this.el.querySelector('#tts-toggle');
        ...
        const gemmaToggle = this.el.querySelector('#gemma-toggle');
    ...
    async handleDeleteFarmer(name) {
        ...
        this.defaults = {
            ...
            defaultFarmerName: this.defaults.defaultFarmerName === name
                ? (newNames[0] ?? '')
                : this.defaults.defaultFarmerName,
        };
        this.renderFarmerList();
    }
    handleSetDefaultFarmer(name) {
    ...
    handleDeleteLabel(label) {
        ...
        this.defaults = {
            ...this.defaults,
            labels: newLabels,
            defaultLabel: this.defaults.defaultLabel === label
                ? (newLabels[0] ?? '')
                : this.defaults.defaultLabel,
        };
        this.renderLabelChips();
    }
    handleSetDefaultLabel(label) {
    ...
    handleDeleteTreatment(treatment) {
        ...
        this.defaults = {
            ...
            defaultTreatment: this.defaults.defaultTreatment === treatment
                ? (newTreatments[0] ?? '')
                : this.defaults.defaultTreatment,
        };
        this.renderTreatmentChips();
    }
    handleSetDefaultTreatment(treatment) {
    ...
    async handleSave() {
        if (this.isSaving)
            return;
        // 조사나무 범위 읽기
        const minInput = this.el?.querySelector('#tree-min-input');
        const maxInput = this.el?.querySelector('#tree-max-input');
