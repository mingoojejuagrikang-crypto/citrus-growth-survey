 ...
 * @returns 스토어 이름
 */
function getStoreName(type: SurveyType): 'growthRecords' | 'qualityRecords' {
...
export async function saveRecord(
  type: SurveyType,
  record: GrowthRecord | QualityRecord,
): Promise<void> {
  try {
    const db = await getDB();
    const storeName = getStoreName(type);
    const now = nowIso();
    const recordToSave: GrowthRecord | QualityRecord = {
      ...record,
      syncStatus: 'pending' as SyncStatus,
      updatedAt: now,
    };
    await db.put(storeName, recordToSave as GrowthRecord & QualityRecord);
  } catch (err) {
    throw new DBError(
      `레코드 저장 실패 (type=${type}, id=${record.id}): ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function getRecordById(
  type: SurveyType,
  id: string,
): Promise<GrowthRecord | QualityRecord | undefined> {
  try {
    const db = await getDB();
    const storeName = getStoreName(type);
    ...
    throw new DBError(
      `레코드 조회 실패 (type=${type}, id=${id}): ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function getRecords(
  type: SurveyType,
  filter?: RecordFilter,
): Promise<(GrowthRecord | QualityRecord)[]> {
  try {
    const db = await getDB();
    const storeName = getStoreName(type);

    let records: (GrowthRecord | QualityRecord)[];
    ...
    throw new DBError(
      `레코드 목록 조회 실패 (type=${type}): ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function updateSyncStatus(
  ...
  try {
    ...
    const now = nowIso();
    const updated: GrowthRecord | QualityRecord = {
      ...record,
      syncStatus: status,
      syncedAt: status === 'synced' ? now : record.syncedAt,
      updatedAt: now,
    };
    await db.put(storeName, updated as GrowthRecord & QualityRecord);
  } catch (err) {
    if (err instanceof DBError) throw err;
    throw new DBError(
      `동기화 상태 갱신 실패 (type=${type}, id=${id}): ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
 ...
 */
export async function deleteRecord(type: SurveyType, id: string): Promise<void> {
  try {
    const db = await getDB();
    const storeName = getStoreName(type);
    ...
    throw new DBError(
      `레코드 삭제 실패 (type=${type}, id=${id}): ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function getPendingCount(): Promise<number> {
  try {
    ...
    throw new DBError(
      `미동기화 건수 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function getRecentSessions(limit = 5): Promise<SessionSummary[]> {
  try {
    ...
    const sessionMap = new Map<string, GrowthRecord | QualityRecord>();
    ...
    throw new DBError(
      `최근 세션 목록 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
...
export async function getPendingRecords(): Promise<{
  ...
  try {
    ...
    throw new DBError(
      `미동기화 레코드 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
      err,
    );
  }
}
