...
export async function get(key, defaultValue) {
    try {
        ...
        throw new Error(`설정 조회 실패 (key=${key}): ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
 * 설정값을 저장합니다.
...
export async function set(key, value) {
    try {
        ...
        throw new Error(`설정 저장 실패 (key=${key}): ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
...
export async function getDefaults() {
    return get('defaults', DEFAULT_APP_DEFAULTS);
}
/**
 * AppDefaults를 저장합니다.
 *
...
export async function resetToDefaults() {
    try {
        ...
        const puts = [
            ...
            ...Object.entries(SETTING_DEFAULTS).map(([key, value]) => db.put('settings', { key, value })),
        ];
        await Promise.all(puts);
    }
    catch (err) {
        throw new Error(`설정 초기화 실패: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
 ...
 */
export function getDefaultAppDefaults() {
