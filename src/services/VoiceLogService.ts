...
import type { VoiceLog, VoiceAudio, LogFilter, LogDateStat } from '../types.js';
 ...
 */
export async function saveLog(log: Omit<VoiceLog, 'id'>): Promise<string> {
  try {
    const db = await getDB();
    const id = generateId();
    const record: VoiceLog = { id, ...log };
    await db.add('voiceLogs', record);
    return id;
  } catch (err) {
    throw new Error(
      `음성 로그 저장 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
 ...
 * @throws Error 저장 실패 시
 */
export async function saveAudio(audio: Omit<VoiceAudio, 'id'>): Promise<string> {
  try {
    const db = await getDB();
    const id = generateId();
    const record: VoiceAudio = { id, ...audio };
    await db.add('voiceAudio', record);
    return id;
  } catch (err) {
    throw new Error(
      `오디오 저장 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
 ...
 */
export async function getLogs(filter?: LogFilter): Promise<VoiceLog[]> {
  try {
    const db = await getDB();
    let logs: VoiceLog[];
    ...
    if (filter?.dateFrom !== undefined || filter?.dateTo !== undefined) {
      logs = logs.filter((l) => {
        ...
        if (filter.dateFrom !== undefined && datePrefix < filter.dateFrom) return false;
        if (filter.dateTo !== undefined && datePrefix > filter.dateTo) return false;
        return true;
      });
    }
    ...
    throw new Error(
      `음성 로그 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function deleteLog(id: string): Promise<void> {
  try {
    ...
    throw new Error(
      `음성 로그 삭제 실패 (id=${id}): ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function deleteAudio(audioId: string): Promise<void> {
  try {
    ...
    throw new Error(
      `오디오 삭제 실패 (id=${audioId}): ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function deleteLogsByDateRange(
  ...
  try {
    ...
    throw new Error(
      `날짜 범위 로그 삭제 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function deleteAllLogs(): Promise<void> {
  try {
    ...
    throw new Error(
      `전체 로그 삭제 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function exportLogs(filter?: LogFilter): Promise<string> {
  try {
    ...
    throw new Error(
      `로그 JSON 내보내기 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
 ...
 */
export async function exportLogsWithAudio(filter?: LogFilter): Promise<Uint8Array> {
  try {
    const db = await getDB();
    const logs = await getLogs(filter);
    ...
    const jsonStr = JSON.stringify(
      { exportedAt: nowIso(), count: logs.length, logs },
      null,
      2,
    );
 ...
 * @throws Error 조회 실패 시
 */
export async function getLogStats(): Promise<LogDateStat[]> {
  try {
    const db = await getDB();
    ...
    for (const log of logs) {
      ...
      stat.totalCount++;
      if (log.kind === 'ok') stat.okCount++;
      else if (log.kind === 'warn') stat.warnCount++;
      else if (log.kind === 'fail') stat.failCount++;
      if (log.audioFileId !== null) stat.audioIds.add(log.audioFileId);
    }
    ...
    throw new Error(
      `로그 통계 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export async function getStorageStats(): Promise<{
  ...
  try {
    ...
    throw new Error(
      `저장소 통계 조회 실패: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
