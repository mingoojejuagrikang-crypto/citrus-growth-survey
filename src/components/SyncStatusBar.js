...
export class SyncStatusBar {
    ...
    mount(container) {
        ...
        // SyncStore 구독
        this.unsubscribers.push(syncStore.subscribe((state) => {
            const appState = appStore.getState();
            this.updateUI(state, appState);
        }));
        // AppStore 구독 — 온라인/오프라인 상태에 따라 버튼 활성화
        this.unsubscribers.push(appStore.subscribe((appState) => {
    ...
    `;
        const btn = this.el.querySelector('.sync-btn');
